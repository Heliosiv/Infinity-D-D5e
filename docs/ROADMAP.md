...
source checks pass. The disposable Foundry world has now been recovered;
private hunting/Research continuity and daily living now have bounded native
acceptance, including GM handoff, pause, large jumps, skip and lost-write-reply
recovery. Research reaches a reviewed receipt on the authenticated player client. See the
...
| Private data                | `private-state.js` integrates the automatic trusted-table campaign store                                                     | GM UI permissions remain; deliberate authenticated-client inspection is an accepted trusted-table boundary.                                             |
