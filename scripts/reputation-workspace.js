...
export class ReputationWorkspaceApp extends HandlebarsApplicationMixin(
  ...
  static open() {
    ...
    if (!ReputationWorkspaceApp._instance) {
      ReputationWorkspaceApp._instance = new ReputationWorkspaceApp();
    }
    if (ReputationWorkspaceApp._instance.rendered) {
      ReputationWorkspaceApp._instance.bringToFront();
    } else {
      ReputationWorkspaceApp._instance.render(true);
    }
    return ReputationWorkspaceApp._instance;
  }
  ...
  _onRender(context, options) {
    ...
    this.element?.classList?.toggle("rw-no-anim", !animations);
  ...
  static async _onDeleteFaction() {
    ...
    const confirmed = DialogV2
      ? await DialogV2.confirm({
          ...
          content: `<p>This permanently removes <strong>${escapeText(faction.name)}</strong> and its reputation history. This can't be undone.</p>`,
          rejectClose: false,
        })
      : true;
    if (!confirmed) return;
...
async function promptReason(title, hint) {
  ...
  try {
    const value = await DialogV2.prompt({
      ...
      content: `
        <div class="rw-prompt">
          <p class="rw-prompt__hint">${escapeText(hint)}</p>
