 ...
 * ------------------------------------------------------------------ */
{
  ...
  const allowed = new Set(["wand1", "wand2", "wand3"].map((id) => `Compendium.test.Item.${id}`));
  ...
  assert.equal(new Set(rows.map((r) => r.uuid)).size, rows.length, "distinct items");
}
 ...
 * ------------------------------------------------------------------ */
{
  ...
  const commonUuids = new Set(["gem1", "gem2"].map((id) => `Compendium.test.Item.${id}`));
...
{
  const ammo = [
    mkItem("arrows", "consumable", "common", { system: { type: { value: "ammo" }, price: { value: 1, denomination: "gp" } } }),
    mkItem("bolts", "consumable", "common", { system: { type: { value: "ammo" }, price: { value: 1, denomination: "gp" } } }),
  ];
  const { rows } = rollMerchantStock(
 ...
 * ------------------------------------------------------------------ */
{
  const exclude = new Set(["wand1", "wand2"].map((id) => `Compendium.test.Item.${id}`));
  const { rows } = rollMerchantStock(
