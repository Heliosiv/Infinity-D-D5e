/**
 * Same-browser leadership for authoritative campaign writes.
 *
 ...
 * campaign writes. IndexedDB supplies the fallback on ordinary HTTP/LAN pages.
 * Cross-device concurrency remains a documented Foundry/CAS limitation.
 */

import { browserLeaseManager } from "./browser-lock.js";

const MODULE_ID = "infinity-dnd5e";
export const CAMPAIGN_TAB_LEADERSHIP_HOOK =
  "infinity-dnd5e.campaignTabLeadership";
const LEGACY_MERCHANT_TAB_LEADERSHIP_HOOK =
  "infinity-dnd5e.merchantTabLeadership";
const DEFAULT_DECISION_WAIT_MS = 25;
const DEFAULT_FALLBACK_LEASE_TTL_MS = 30_000;

export function createCampaignTabLeadership({
...
function lockName(scope) {
  // Keep the established physical key until a future dual-lock migration can
  // prove that hot-reloaded/older tabs cannot lead concurrently.
  return `${MODULE_ID}:merchant-authority:${scope}`;
}

function defaultNotify(value) {
  globalThis.Hooks?.callAll?.(CAMPAIGN_TAB_LEADERSHIP_HOOK, value);
  globalThis.Hooks?.callAll?.(LEGACY_MERCHANT_TAB_LEADERSHIP_HOOK, value);
}

const campaignTabLeadership = createCampaignTabLeadership();

export function ensureCampaignTabLeadership() {
  return campaignTabLeadership.ensureLeadership();
}

export function hasCampaignTabLeadership() {
  return campaignTabLeadership.hasLeadership();
}

export function getCampaignTabLeadershipStatus() {
  return campaignTabLeadership.status();
}

/**
 * Release the campaign-wide singleton during the owning application lifecycle.
 * Feature subsystems share this leader and must not release it independently.
 */
export function releaseCampaignTabLeadership() {
  return campaignTabLeadership.releaseLeadership();
}
