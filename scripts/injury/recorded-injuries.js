...
/** Records already represented by an automated effect must not count twice. */
export function getStandaloneRecordedInjuryRows(actor) {
  const automatedSources = new Set(
    values(actor?.effects)
      .filter((effect) => effect?.flags?.[MODULE_ID]?.criticalInjury?.id)
      .map((effect) => effect.uuid)
      .filter(Boolean),
  );
  return getRecordedInjuryRows(actor).filter(
    (record) => !automatedSources.has(record.sourceUuid),
  );
}
