...
async function writeNotes(manifest) {
  ...
  await writeFile(
    ...
    [
      ...
      "## Verify",
      "",
      "After installing, enable the module in your world. Open the role-aware Home from either entry point:",
      "",
      "- The single **Infinity D&D5e** d20 category in scene controls",
      "- **Shift+I** from anywhere in the game",
      "",
      "Home shows each user only the destinations their role is allowed to open.",
      "Players can also open **Downtime Activities** with **Shift+D** once the module is enabled.",
      "",
    ].join("\n"),
    "utf8",
  );
