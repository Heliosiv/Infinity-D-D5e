...
- **Hoard Loot** (available) — a single treasure cache. **Threat Tier** + **Hoard Scale** (Small / Standard / Large / Massive) set the gp budget; the **Pile Bias** slider trades raw coin for items. Rarity chips auto-tune to the scale's narrative shape: Small = pocket stash (low rarity), Standard = locked cache, Large = armory or vault, Massive = end-dungeon prize (artifact-eligible at T4+). Defaults are **sticky** — your manual chip tweaks survive a tier/scale comparison click. Item count is a soft ceiling tucked under the chips. Result presents as a coin-pile card (with `pp / gp / sp / cp` breakdown) plus the item list.
